      ...
      async (firebaseUser) => { // Auth state determined
        if (firebaseUser) {
          ...
          if (!userSnap.exists()) {
            ...
            const displayName = firebaseUser.displayName || firebaseUser.email?.split('@')[0] || 'Anonymous';
            const userData = {
                id: firebaseUser.uid,
            ...
            // Set the doc non-blockingly
            setDocumentNonBlocking(userRef, userData, { merge: true });
            
            // Also update the auth profile if the display name was missing
            if (!firebaseUser.displayName) {
                try {
                    await updateProfile(firebaseUser, { displayName: displayName });
                } catch (e) {
                    console.error("Failed to update auth profile display name:", e);
                }
            }
          }
        }
